...
def create_and_clear_data(request, set_api, generate_bucket_name):
    request.session.api.put_bucket(request.session.bucket_name)
    ...
    yield
    request.session.api.delete_object(request.session.bucket_name, request.param['file_key'])
    request.session.api.delete_bucket(request.session.bucket_name)
